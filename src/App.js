...
function Pill({ habitId, small, habits, t }) {
  const h = habits.find(x=>x.id===habitId);
  if (!h) return <span style={{ padding:small?"2px 7px":"3px 9px", borderRadius:20, fontSize:9, background:t.bgCard, color:t.textDim, letterSpacing:"0.04em", whiteSpace:"nowrap", border:`1px solid ${t.borderCard}` }}>PERSONAL</span>;
  return <span style={{ padding:small?"2px 7px":"3px 9px", borderRadius:20, fontSize:9, background:h.color+"22", color:h.color, border:`1px solid ${h.color}44`, letterSpacing:"0.04em", fontWeight:700, display:"inline-flex", alignItems:"center", gap:3, whiteSpace:"nowrap" }}>{h.icon} {h.label.toUpperCase()}</span>;
}
...
function DraggableList({ items, onReorder, dayKey, editingBlock, editText, setEditText, onEditTask, saveEdit, habits, t }) {
  ...
  const onDragStart = (e,i) => {
    ...
    const g=document.createElement("div"); g.style.cssText="position:absolute;top:-9999px;";
  ...
  const onTouchStart = (e,i) => {
    ...
    touchRef.current={active:true,startIdx:i,ghost:null};
    setDragIdx(i);
    const el=itemRefs.current[i];
    if (el) {
      const g=el.cloneNode(true), r=el.getBoundingClientRect();
      g.style.cssText=`position:fixed;pointer-events:none;opacity:0.92;z-index:9999;width:${r.width}px;left:${r.left}px;top:${touch.clientY-r.height/2}px;border:1px solid ${t.accent}99;border-radius:7px;box-shadow:0 8px 30px rgba(0,0,0,0.4);background:${t.ghostBg};`;
      document.body.appendChild(g); touchRef.current.ghost=g;
    }
  };
  const onTouchMove = e => {
    if (!touchRef.current.active) return; e.preventDefault();
    const y=e.touches[0].clientY;
    if (touchRef.current.ghost) touchRef.current.ghost.style.top=`${y-24}px`;
    setOverIdx(idxFromY(y));
  };
  const onTouchEnd = () => {
    if (!touchRef.current.active) return;
    doSwap(touchRef.current.startIdx,overIdx);
    if (touchRef.current.ghost){document.body.removeChild(touchRef.current.ghost);}
    touchRef.current={active:false,startIdx:null,ghost:null};
        ...
        return (
          <div key={idx} ref={el=>itemRefs.current[idx]=el}
            ...
            style={{
              ...
              background:isDragging?t.accent+"11":isOver?t.dragOver:h?`${h.color}0d`:t.bgCard,
              border:`1px solid ${isDragging?t.accent+"55":isOver?t.accent+"35":h?h.color+"28":t.borderCard}`,
              borderRadius:7,opacity:isDragging?0.35:1,
              transform:isOver?"translateY(-2px)":"none",
              boxShadow:isOver?`0 4px 18px rgba(0,0,0,0.2)`:"none",
              transition:isDragging?"none":"transform 0.12s,box-shadow 0.12s,border-color 0.12s",
            ...
            {isEditing?(
              <input value={editText} onChange={e=>setEditText(e.target.value)} onBlur={saveEdit} onKeyDown={e=>e.key==="Enter"&&saveEdit()} autoFocus
                style={{flex:1,background:"transparent",border:"none",borderBottom:`1px solid ${t.accent}`,color:t.text,fontSize:12,fontFamily:"'Courier New',monospace",outline:"none",padding:"2px 0"}}/>
            ):(
            ...
            <Pill habitId={block.habit} small habits={habits} t={t}/>
          </div>
        );
      })}
    </div>
...
export default function App() {
  ...
  const habits = isDark ? HABITS_DARK : HABITS;

  const [view, setView] = useState("today");
  const [habitLog, setHabitLog] = useLS("habitlog-v1", {});
  const [weekChecks, setWeekChecks] = useLS("weekcheck-v1", {});
  const [schedule, setSchedule] = useLS("schedule-v1", DEFAULT_SCHEDULE);
  const [todayOverride, setTodayOverride] = useLS("todayoverride-v1", {});
  ...
  const onEditTask=(dayKey,idx)=>{
    const items=dayKey==="TODAY"?todayBlocks:(schedule[dayKey]||[]);
    setEditingBlock({day:dayKey,idx}); setEditText(items[idx]?.task||"");
  };
  const saveEdit=()=>{
    if (!editingBlock) return;
    const {day,idx}=editingBlock;
    if (day==="TODAY") setTodayOverride({...todayOverride,[TODAY]:todayBlocks.map((b,i)=>i===idx?{...b,task:editText}:b)});
    else setSchedule({...schedule,[day]:schedule[day].map((b,i)=>i===idx?{...b,task:editText}:b)});
  ...
  const dayBtn=(active)=>({...navBtn(active)});

  const sharedList = { habits, t };
    ...
    <div style={{minHeight:"100vh",background:t.bg,color:t.text,fontFamily:"'Courier New',monospace",paddingBottom:70,transition:"background 0.3s,color 0.3s"}}>
      ...
      <div style={{padding:"16px 16px"}}>
        ...
        {view==="today"&&<>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:24}}>
            {habits.map(h=>{
              ...
              return (
                <div key={h.id} onClick={()=>setHabitLog({...habitLog,[TODAY]:{...(habitLog[TODAY]||{}),[h.id]:!done}})} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 13px",background:done?`${h.color}16`:t.bgCard,border:`1px solid ${done?h.color+"44":t.borderCard}`,borderRadius:7,cursor:"pointer",transition:"all 0.2s"}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
            ...
            {todayOverride[TODAY]&&<button onClick={()=>{const o={...todayOverride};delete o[TODAY];setTodayOverride(o);}} style={{fontSize:9,color:t.textMid,background:"transparent",border:`1px solid ${t.inputBorder}`,borderRadius:4,padding:"3px 8px",cursor:"pointer",fontFamily:"'Courier New',monospace"}}>RESET</button>}
          </div>
          <div style={{fontSize:10,color:t.textFaint,marginBottom:10}}>Changes here are today only</div>
          <DraggableList items={todayBlocks} onReorder={items=>setTodayOverride({...todayOverride,[TODAY]:items})} dayKey="TODAY" editingBlock={editingBlock} editText={editText} setEditText={setEditText} onEditTask={onEditTask} saveEdit={saveEdit} {...sharedList}/>
        </>}
        ...
        {view==="week"&&<>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:24}}>
            {NON_NEGS.map(nn=>{
              const done=!!curWeek[nn.id], h=habits.find(x=>x.id===nn.habit);
              return (
                <div key={nn.id} onClick={()=>setWeekChecks({...weekChecks,[weekId]:{...(weekChecks[weekId]||{}),[nn.id]:!done}})} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 13px",background:done?t.accent+"0d":t.bgCard,border:`1px solid ${done?t.accent+"30":t.borderCard}`,borderRadius:7,cursor:"pointer",transition:"all 0.2s"}}>
                  ...
                  {h&&<Pill habitId={h.id} small habits={habits} t={t}/>}
                </div>
              );
            })}
          </div>
          ...
          {DAYS.map(day=>(
            <div key={day} style={{marginBottom:15}}>
              ...
              <StaticList items={day===TODAY_DAY?todayBlocks:(schedule[day]||[])} {...sharedList}/>
            </div>
          ))}
        </>}
        ...
        {view==="calendar"&&<>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3,marginBottom:20}}>
            ...
            {Array.from({length:numDays}).map((_,i)=>{
              ...
              const score=dayScore(ds), isToday=ds===TODAY, isFuture=ds>TODAY;
              return (
                <div key={day} style={{aspectRatio:"1",borderRadius:5,background:isFuture?t.calEmpty:scoreColor(score),border:isToday?`2px solid ${t.accent}`:t.calBorder?`1px solid ${t.calBorder}`:"none",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:7}}>
            {habits.map(h=>{
              const count=Array.from({length:numDays},(_,i)=>{const ds=`${year}-${String(month+1).padStart(2,"0")}-${String(i+1).padStart(2,"0")}`;return(habitLog[ds]||{})[h.id]?1:0;}).reduce((a,b)=>a+b,0);
              return (
        ...
        {view==="schedule"&&<>
          ...
          <div style={{display:"flex",gap:4,marginBottom:16,flexWrap:"wrap"}}>
            {DAYS.map(d=>(
              <button key={d} onClick={()=>setActiveDay(d)} style={dayBtn(activeDay===d)}>{d}{d===TODAY_DAY?" ·":""}</button>
            ))}
          </div>
          <DraggableList items={schedule[activeDay]||[]} onReorder={items=>setSchedule({...schedule,[activeDay]:items})} dayKey={activeDay} editingBlock={editingBlock} editText={editText} setEditText={setEditText} onEditTask={onEditTask} saveEdit={saveEdit} {...sharedList}/>
        </>}
